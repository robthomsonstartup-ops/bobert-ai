...
- ✅ **Parallel source extraction (commit `0473b34`)** — `Promise.all` across OCR/PDF/URL; all attachments in a bid packet now extract simultaneously before AI synthesis. Closes speed-to-structured-output workstream.
- ✅ **Inline field editing (commit `0473b34`)** — Edit/Done toggle on Project Info, Quote Checklist, Deal Draft cards; `contenteditable` fields write back to `currentIntake` on blur; checklist items cycle present/missing/unknown in edit mode.
- ✅ **UI polish (current sprint)** — header logo fixed (R1b: `icon.png` → `bobert-wordmark-white.png`), action strip consolidated to 4-button grid.
- 🔲 Remaining UI polish — visual refinements, input flow continuations (next sprint as real-world usage at CSI reveals gaps)
