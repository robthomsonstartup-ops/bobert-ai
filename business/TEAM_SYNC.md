...
- ✅ Multi-source intake shipped — drop zone (images/PDFs), clipboard paste (screenshots), URL fetch; all sources extracted to text and combined before AI call. New endpoints: `api/intake-ocr.js` (Google Vision), `api/intake-url.js` (server-side fetch + HTML strip)
